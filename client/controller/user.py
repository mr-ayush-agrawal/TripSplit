...
async def signup_handler(name : str, email : str, user_name : str, password : str, re_password: str, currency : str = None):
    try : 
...
async def login_handler(request: Request, email: str = None, user_name: str = None, password: str = Form(...)):
    try : 
...
async def personal_update_handler(request: Request, name: str = Form(...)):
    try:
        auth_cookie = request.cookies.get(LOGIN_COOKIE_NAME)
        if not auth_cookie:
            return RedirectResponse(url="/user/login", status_code=303)
...
async def account_update_handler(request: Request, email: str = Form(...), currency: str = Form(...)):
    try:
        auth_cookie = request.cookies.get(LOGIN_COOKIE_NAME)
        if not auth_cookie:
            return RedirectResponse(url="/user/login", status_code=303)
...
async def password_update_handler(request : Request, old_password: str = Form(...), new_password : str = Form(...), confirm_password: str = Form(...)):
    try:
